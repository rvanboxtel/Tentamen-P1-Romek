            ...
            <div class="row card-group mt-3">
                <div class="card bg-light mb-3">
                    <div class="card-header">
                        <a class="btn btn-secondary float-left" href="<?= $prefix ?>/users" role="button">terug naar alle
                            gebruikers</a>
                        ...
                        <form action="<?= $prefix ?>/users/update" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                ...
                                <select id="role" name="role_id" class="form-control js-select2" required>
                                    <?php foreach ($roles::getAll() as $role): ?>
                                        <option class="form-control" value="<?= $role->getId() ?>"><?= $role->getDescription() ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="dateOfBirth" name="dateOfBirth"
                                       value="<?= $user->getDateOfBirth() ?>" required>
                            </div>

                            <div class="form-group">
                                <label for="reason">reason:</label>
                                <input type="text" class="form-control" id="reason" name="reason"
                                       value="<?= $user->getReason() ?>" required>
                            </div>
